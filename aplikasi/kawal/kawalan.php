...
class Kawalan extends Kawal 
{
    ...
    {    
        ...
        {// mula ulang table
            ...
            $this->lihat->bilSemua[$myTable] = $bilSemua;
            // sql guna limit
            $this->lihat->cariApa[$myTable] = $this->tanya->
            paparSemua($sv, $myTable, $medan, $fe, $jum);
            // halaman
            $this->lihat->halaman[$myTable] = halaman($jum);
        }// tamat ulang table

		# semak pembolehubah $this->lihat->cariApa
        //echo '<pre>', print_r($this->lihat->cariApa, 1) . '</pre><br>';
        
        // papar
        $this->lihat->pegawai = senarai_kakitangan();
        $this->lihat->carian = 'semuajadual';
        $this->lihat->baca('kawalan/index', 0);
    }
    ...
    {    
        ...
        // Set pemboleubah utama
        $this->lihat->pegawai = senarai_kakitangan();
        $this->lihat->carian = 'belum';
        $this->lihat->url = dpt_url();
        // pergi papar kandungan
        $this->lihat->baca('kawalan/index');
    }
    ...
    {//echo '<br>Anda berada di class Imej extends Kawal:ubah($cari)<br>';
        ...
        $this->lihat->kesID = array();

		
        if (!empty($id)) 
        {
            //echo '$id:' . $id . '<br>';
            $this->lihat->carian='newss';	
        ...
        {
            $this->lihat->carian='[tiada id diisi]';
        }
        ...
        $this->lihat->paparTajuk = null;
        $s1 = '<span class="label">';
        $s2 = '</span>';
        foreach ($tajuk2 as $tajuk)
        {
            $this->lihat->paparTajuk .= "\n" . '<th>' . $s1 . $tajuk . $s2 . '</th>';
        }

        $this->lihat->pegawai = senarai_kakitangan();
        $this->lihat->lokasi = 'MDT 2013 - papar';
		$this->lihat->cari = $id;
    ...
    {    
        ...
        // Set pemboleubah utama
        $this->lihat->pegawai = senarai_kakitangan();
        $this->lihat->carian = 'semakbatch';
        $this->lihat->url = dpt_url();
        // pergi papar kandungan
        $this->lihat->baca('kawalan/semak', 0);
    }
    ...
    function cari() 
    {
        ...
        {
            $this->lihat->carian='[id:0]';
        }
        
        // paparkan ke fail cimej/cari.php
        $this->lihat->baca('cimej/cari');
    ...
    {//echo '<br>Anda berada di class Imej extends Kawal:ubah($cari)<br>';
        ...
        if (!empty($id)) 
        {
            ...
            $this->lihat->rangka['kes'] = 
                $this->tanya->cariSemuaMedan($sv . $jadualRangka, 
                $medanRangka, $cari);
			
			// 1.1 ambil nilai msic & msic08
			$msic00 = $this->lihat->rangka['kes'][0]['msic'];
			//$msic08 = $this->lihat->rangka['kes'][0]['msic08'];
			$cariM6['medan'] = 'msic';
			$cariM6['id'] = $msic00;
			...
			{// mula ulang table
				...
				$this->lihat->cariIndustri[$msic] = $this->tanya->
            ...
            foreach ($bulanan as $key => $myTable)
            {// mula ulang table
                $this->lihat->kesID[$myTable] = 
                    $this->tanya->cariSemuaMedan($sv . $myTable, 
                    $medanData, $cari);
				$hasil[$key] = isset($this->lihat->kesID[$myTable][0]['hasil']) ?
					$this->lihat->kesID[$myTable][0]['hasil'] : 0;
				$dptLain[$key] = isset($this->lihat->kesID[$myTable][0]['dptLain']) ?
					$this->lihat->kesID[$myTable][0]['dptLain'] : 0;
				// masukkan $data untuk dapatkan perbezaan data bulanan
				$data['dpt'][$key] = $hasil[$key] + $dptLain[$key];
				$data['hasil'][$key] = isset($this->lihat->kesID[$myTable][0]['hasil']) ?
					$this->lihat->kesID[$myTable][0]['hasil'] : 0;
				$data['dptLain'][$key] = isset($this->lihat->kesID[$myTable][0]['dptLain']) ?
					$this->lihat->kesID[$myTable][0]['dptLain'] : 0;
				$data['web'][$key] = isset($this->lihat->kesID[$myTable][0]['web']) ?
					$this->lihat->kesID[$myTable][0]['web'] : 0;
				$data['stok'][$key] = isset($this->lihat->kesID[$myTable][0]['stok']) ?
					$this->lihat->kesID[$myTable][0]['stok'] : 0;
				$data['staf'][$key] = isset($this->lihat->kesID[$myTable][0]['staf']) ?
					$this->lihat->kesID[$myTable][0]['staf'] : 0;
				$data['gaji'][$key] = isset($this->lihat->kesID[$myTable][0]['gaji']) ?
					$this->lihat->kesID[$myTable][0]['gaji'] : 0;
            }// tamat ulang table
			...
			{// mula ulang data
				...
				{// mula ulang table
					...
					$this->lihat->beza[$myTable][0][$medan] = ($key==0) ? null : 
						//kira($kini)	. '|' . 
						kira2($dulu,$kini);
				}// tamat ulang table
			}// tamat ulang data
		...
		//echo '$beza::'; print_r($beza); 
		//echo '$this->lihat->rangka:<br>'; print_r($this->lihat->rangka); 
		echo '$this->lihat->cariIndustri:<br>'; print_r($this->lihat->cariIndustri); 
		//echo '$this->lihat->cari:<br>'; print_r($this->lihat->cari); 
		echo '</pre>';
		//*/
		
        // paparkan ke fail kawalan/ubah.php
		$this->lihat->baca('kawalan/ubah', 0);
	...
	{				
		...
		if (!empty($id)) 
        {
			...
			//$this->lihat->dataRangka[$dtsample] = $this->tanya->semakRangkaProsesan($mytable = 'dtsample_takatfeb13', $medan = '*', $cari);
			// papar rangka mdt 2013 sahaja
			$paparRangka[$dtsample] = $this->tanya->paparMedan($dtsample, $papar = null);
			$this->lihat->paparRangka[$dtsample][] = Borang::tambahRangkaBaru($paparRangka, $dataRangka);
			...
			{// mula ulang table
				...
				$this->lihat->paparMedan[$sv . $myTable][] = Borang::tambahBaru($paparMedan, $databln);
			}// tamat ulang table
		...
		echo '$$this->lihat->paparRangka:'; print_r($this->lihat->paparRangka) . '';
		echo '</pre>';
		//*/

		// Set pemboleubah utama
        $this->lihat->pegawai = senarai_kakitangan();
        $this->lihat->lokasi = 'MDT 2013:Tambah';
		// paparkan ke fail kawalan/tambah.php
		$this->lihat->baca($this->_folder . '/tambah', 0);
	}
	...
	public function cetak($cariID, $cetak = null)
	{
		...
		elseif (!empty($cariID)) 
		{
			...
			// mula cari $cariID dalam $jadual
			$this->lihat->cariApa = $this->tanya
				//->cariCantumJadual($jadual, $medan = '*', $kira);
				->cariKawalan($jadual, $cari = 'newss', $cariID);

			//echo '<pre>$cariNama::'; print_r($this->lihat->cariApa) . '</pre>';
			
			$this->lihat->carian=$cariID;
		}
		else
		{
			$this->lihat->carian[]='[id:0]';
	...
	public function papar($cariID, $cetak = null)
	{
		$this->lihat->cariNama = null;
		...
		{	
			...
			//echo '<pre>$cariApa::'; print_r($this->lihat->cariNama) . '</pre>';
